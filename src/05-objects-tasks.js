...
class Builder {
  ...
  attr(value) {
    ...
    this.selectors.set('attributes', [
      ...(this.selectors.has('attributes') ? this.selectors.get('attributes') : []),
      `[${value}]`,
    ]);
    return this;
  }
  ...
  pseudoClass(value) {
    ...
    this.selectors.set('pseudoClasses', [
      ...(this.selectors.has('pseudoClasses') ? this.selectors.get('pseudoClasses') : []),
      `:${value}`,
    ]);
    return this;
  }
  ...
  stringify() {
    ...
    const res = o.reduce((a, c) => {
      if (s.has(c)) {
        return typeof s.get(c) === 'object' ? a + s.get(c).join('') : a + s.get(c);
      }
      return a;
    }, '');
    this.selectors = new Map();
